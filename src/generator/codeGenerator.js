...
const buildGroup = (endpointGroup, getRelativePath) => {
    ...
    return {
        ...
        controller: controllers.map(c => c.content).join('\n\n'),
...
export const generateAPICode = (
    ...
    const groups = allEndpointsGroups.map(groups => buildGroup(groups, getRelativePath));

    return {
        code: 'Código generado de la API',
        envContent: generateEnvFile(useEnvironmentVariables, databaseConfig),
        groups,
        index: generateIndex(groups.map(({ groupName, path }) => ({ groupName, path }))),
    };
};
